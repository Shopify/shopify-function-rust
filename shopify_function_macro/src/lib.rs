...
use bluejay_typegen_codegen::{
    generate_schema, Codec, Input as BluejayInput, KnownCustomScalarType,
};
    ...
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        while !input.is_empty() {
            ...
            if lookahead.peek(kw::input_stream) {
                ...
                args.codec = Some(Self::parse::<kw::codec, Codec>(&input)?);
            } else {
...
/// - If the `codec` argument is provided, it will use the specified codec for
...
pub fn shopify_function(
    ...
    let codec = args.codec.unwrap_or(Codec::Serde);

    let deserialize_line: syn::Stmt = match codec {
        Codec::Serde => syn::parse_quote! {
            let input: #input_type = shopify_function::serde_json::from_str(&string).unwrap();
        },
        Codec::Miniserde => syn::parse_quote! {
            let input: #input_type = shopify_function::miniserde::json::from_str(&string).unwrap();
        },
    };

    let serialize_line: syn::Stmt = match codec {
        Codec::Serde => syn::parse_quote! {
            let serialized = shopify_function::serde_json::to_vec(&result).unwrap();
        },
        Codec::Miniserde => syn::parse_quote! {
            let serialized = shopify_function::miniserde::json::to_string(&result);
        },
    };

    let write_line: syn::Stmt = match codec {
        Codec::Serde => syn::parse_quote! {
            std::io::Write::write_all(&mut out, serialized.as_slice()).unwrap();
        },
        Codec::Miniserde => syn::parse_quote! {
            std::io::Write::write_all(&mut out, serialized.as_bytes()).unwrap();
        },
    };
...
// mostly copied from bluejay_typegen_codegen
...
pub fn typegen(
    ...
    input.serde_path = Some(syn::parse_quote! { ::shopify_function::serde });
    input.miniserde_path = Some(syn::parse_quote! { ::shopify_function::miniserde });
    let mut module = syn::parse_macro_input!(item as syn::ItemMod);

    let string_known_custom_scalar_type = KnownCustomScalarType {
        path_for_borrowed: Some(syn::parse_quote! { ::std::borrow::Cow<'a, str> }),
        path_for_owned: syn::parse_quote! { ::std::string::String },
    };
    ...
    let known_custom_scalar_types = HashMap::from([
        ...
        (
            ...
            KnownCustomScalarType {
                path_for_borrowed: None,
                path_for_owned: syn::parse_quote! { () },
            },
        ),
        (
            String::from("Json"),
            KnownCustomScalarType {
                path_for_borrowed: None,
                path_for_owned: syn::parse_quote! { ::shopify_function::serde_json::Value },
            },
        ),
        (
            String::from("Decimal"),
            KnownCustomScalarType {
                path_for_borrowed: None,
                path_for_owned: syn::parse_quote! { ::shopify_function::scalars::Decimal },
            },
        ),
    ]);

    if input.enums_as_str.is_none() {
        let enums_as_str = DEFAULT_EXTERN_ENUMS
            .iter()
            .map(|enum_name| syn::LitStr::new(enum_name, Span::mixed_site()))
            .collect::<Vec<_>>();
        input.enums_as_str = Some(syn::parse_quote! { #(#enums_as_str),* });
        input.allow_unknown_enums_as_str = true;
    }
